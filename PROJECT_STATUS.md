...
- **✅ Order-management batch (07-23) — LIVE on BOTH stores.** Two waves:
  ...
  - **Bilingual EN+AR invoice + letterhead (`fea23c0`, code-only).** The PDF invoice ([invoice.ts](src/lib/invoice.ts)) is now bilingual — Arabic labels/column-headers/totals + the Arabic product name alongside English. **pdfkit renders Arabic correctly by registering the repo's `GE-Dinar-Two-Medium.otf`** — fontkit shapes the joining + RTL reordering, so **no reshaping libraries** (proven: the GSUB yields presentation-form glyphs in visual order, and the PDF embeds the font as CID/Identity-H). Owner uploads a **full-page letterhead** at Admin → Payments → "Invoice letterhead" (Setting `invoice.letterhead`, drawn behind every page, WebP→PNG via sharp; empty = plain header). Manual discount shows as its own titled line. Both invoice routes load + pass the letterhead; `getOrder` selects `nameAr`; letterhead loading isolated in `invoice-letterhead.ts`. ⚠️ **Owner: spot-check one Arabic invoice's layout** (positions verified only via a rendered sample here; the font/shaping is confirmed).
...
- **✅ Checkout payment order + POS rename — LIVE on BOTH stores (`c79fdf4`, owner 2026-07-23).**
  ...
  default is the top card (Card/Kashier)** — the owner reversed an initial COD-default to push card
  payment (`53c471b`); it's `paymentMethods[0]` in the checkout form, following the list's lead. POS
  renamed in the customer label, the checkout i18n (was "Card machine on delivery"), and the admin
  copy label; Arabic labels unchanged. **Verified live on veeey.net AND veeey.com** (both at
  `53c471b`): identical 7-item order, "POS upon Delivery", Card/Kashier pre-ticked. 884 tests, build clean.
