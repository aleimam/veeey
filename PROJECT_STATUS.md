...
## Current state
- **Live** at **veeey.com**. Latest deployed commit: **`948c8e9`** (2026-07-11). All
  **40 Prisma migrations applied** (added `search_rules`, `review_request`, `cart_snapshot`); `pm2` processes `veeey` + `veeey-worker` healthy; `/api/health` → `{"status":"ok"}`.
- **Newest — 4 growth features (2026-07-11):** **#187 Buy-again** (account order history → re-add a past order's in-stock lines to the cart); **#186 Zero-result search fixer** (`/admin/search-rules` — rewrite a query to different terms or redirect it to a page; lists analytics zero-result queries with one-click fixes); **#188 Post-delivery review requests** (order→DELIVERED schedules a delayed email asking for reviews of un-reviewed products; `Order.reviewRequestSentAt` dedupe); **#185 Abandoned-cart recovery** (`/admin/abandoned-carts` + hourly sweep emailing signed-in customers who left items — cookie carts mean **guests can't be captured**). #185/#188 email sends need SMTP configured; toggles in Settings › Cart / Reviews. See assistant memory `veeey-growth-features`.
...
| **Analytics P6 — report builder** | `3044751` | `/admin/analytics/report`: pivot any metric (visitors/pageviews/events/avg-dwell) × dimension (device/country/browser/OS/language/referrer/day/event/page) + optional filter + 7/30/90 range → ranked table with bars + CSV. Config in URL = shareable saved view. `analytics-report.ts` allow-listed SQL fragments (injection-safe) + pure resolveReportConfig guard (+3 tests). Completes the analytics epic. No migration. |
| **Privacy — DSAR erase admin action** | `3044751` | `eraseCustomerAnalyticsAction` (RBAC customers.write + audited) + confirm-gated "Erase analytics data" button in a Privacy & data section on /admin/customers/[id]; wipes a customer's first-party sessions+events for a deletion request (orders untouched). No migration. |
