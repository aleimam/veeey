...
multi-line, photos). A request is **keyed by its `uid`** (`REQ<YY><MM><seq3>`) — the same human
key on both sides — so this endpoint is an **idempotent upsert**: first call creates, later calls
(same `uid`) update in place. This is the write channel for both `requests.create` and
`requests.update` outbox events.
```jsonc
{
  "uid": "REQ2607014",               // REQUIRED — shared correlation key
  "type": "SPECIAL_ORDER",           // SPECIAL_ORDER | OUT_OF_STOCK | RESTOCK | OPTIONAL
  "status": "PENDING",               // PENDING | APPROVED | REJECTED
  "scope": "EGV",
  "notes": "From order VO-1001",     // optional
  "depositEgp": 880,                 // optional (special orders)
  "autoOptional": false,             // true = generated by the monthly Always-Needed refill
  "archived": false,                 // true = request archived on the source side
  // SPECIAL_ORDER: the customer the request is placed for (optional otherwise)
  "customer": { "name": "Ada Lovelace", "phone": "+20100…", "veeeyCustomerId": "cus_1" },
  "veeeyOrderId": "VO-1001",         // optional — the linked pre-order / special-order
  "lines": [                         // ≥ 1; SKU is the shared product key
    { "sku": "VIT-D3-5000", "productName": "Vitamin D3 5000IU", "quantity": 10,
      "sellingPriceEgp": 3500, "notes": null }
  ],
  "photoUrls": [ "https://…/attachments/789" ]   // special-order reference photos
}
```
→ `200 {"uid":"REQ2607014","created":true|false,"unmatchedLines":n}`

- **Money is EGP numbers on the wire** (each app stores minor units internally: Veeey piastres,
  YeldnIN float EGP). Lines whose `sku` is unknown on the receiving side are **dropped** (counted in
  `unmatchedLines`) — push the product first; a request with zero matched lines is skipped.
- Inbound upserts are applied **without re-emitting** (that is what stops the sync echoing back).
- SLA/compensation for SPECIAL_ORDER still flow via §5.1/§5.2 keyed by the same `uid`.
...
### 5.5 `request.upsert` — a request created / updated on the YeldnIN side (Requests epic)
```jsonc
{ "uid":"REQ2607014", "type":"RESTOCK", "status":"PENDING", "scope":"EGV",
  "notes":null, "depositEgp":null, "autoOptional":false, "archived":false,
  "customer":null, "veeeyOrderId":null,
  "lines":[ { "sku":"VIT-D3-5000", "productName":"Vitamin D3 5000IU", "quantity":10,
              "sellingPriceEgp":null, "notes":null } ],
  "photoUrls":[] }
```
The mirror of §4.3, other direction: most requests are placed in Veeey (the easier surface), but a
request placed/edited **in YeldnIN** is pushed here so both sides stay in sync. **Same `payload`
shape as §4.3**, delivered under the §5 envelope (`type:"request.upsert"`). Veeey upserts its
`Request` by `uid`, matching lines to products by SKU (unknown SKUs dropped), and does **not**
re-emit. `request.created` / `request.updated` are accepted as aliases.
...
- **request types** (was "priorities"): `SPECIAL_ORDER, OUT_OF_STOCK, RESTOCK, OPTIONAL`
  (re-baselined — `RESTOCKING` renamed to `RESTOCK`; `OPTIONAL` added for Always-Needed refills)
- **request statuses**: `PENDING, APPROVED, REJECTED` (EGV approval gate)
