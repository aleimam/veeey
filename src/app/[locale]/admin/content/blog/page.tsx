...
export default async function BlogPage({ params, searchParams }: { params: Promise<{ locale: string }>; searchParams: Promise<SP> }) {
  ...
  const all = (await listPosts()).filter((p) => (showingArchived ? p.status === 'ARCHIVED' : p.status !== 'ARCHIVED'));
  const { rows: posts, total } = clientPage(all, lp, { title: (p) => p.titleEn, slug: (p) => p.slug });
  const tf = await getTranslations('admin.fields');
  const tl = await getTranslations('admin.lists');
  const tc = await getTranslations('admin.common');

  const basePath = `/${locale}/admin/content/blog`;
  ...
  return (
    <AdminList
      ...
      bulk={{ formId: 'bulk-blog', action: bulkSoftDeleteAction, locale, back, ops, hidden: { entity: 'post', path: 'content/blog' } }}
