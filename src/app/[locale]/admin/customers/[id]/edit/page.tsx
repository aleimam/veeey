...
export default async function CustomerEditPage({ params, searchParams }: { params: Promise<{ locale: string; id: string }>; searchParams: Promise<SP> }) {
  // Page-level RBAC: editing requires the write grant (the read-only details page
  // only needs customers.read).
  ...
  return (
    <div className="p-6">
      <Link href={`/admin/customers/${c.id}`} className="text-sm text-primary hover:underline">← {tb('Back to customer', 'العودة إلى العميل')}</Link>
      <div className="mb-6 mt-2 flex flex-wrap items-center justify-between gap-3">
        <h1 className="font-heading text-xl font-semibold">
          {tb('Edit', 'تعديل')} · {[c.firstName, c.lastName].filter(Boolean).join(' ') || c.user.name || c.user.email || tb('Customer', 'عميل')}
        </h1>
