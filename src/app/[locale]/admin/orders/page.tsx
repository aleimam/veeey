      ...
      <header className="mb-6 flex flex-wrap items-center justify-between gap-3">
        ...
        <form className="flex flex-wrap items-end gap-2 text-sm">
          ...
          <select name="payCheck" defaultValue={payCheck ?? ''} className={`${inputCls} w-36`}>
            <option value="">Pay check</option>
            {['NO', 'YES', 'PROBLEM'].map((p) => <option key={p} value={p}>{p}</option>)}
          </select>
          <button className="rounded-md border border-border px-3 py-2 hover:bg-surface">Filter</button>
          <a href={`/api/admin/orders/export`} className="rounded-md bg-slate px-3 py-2 font-medium text-slate-foreground">Export CSV</a>
        </form>
      </header>
        ...
        <table className="w-full text-sm">
          <thead className="bg-surface text-xs uppercase text-muted-foreground">
            <tr>
              ...
              <th className="p-3 text-start">Pharmacist</th>
              <th className="p-3 text-start">Pay</th>
              <th className="p-3 text-start">Status</th>
              <th className="p-3 text-start">Items</th>
              <th className="p-3 text-start">Total</th>
              <th className="p-3 text-start">Tracking</th>
              <th className="p-3" />
            </tr>
          </thead>
          <tbody>
            {orders.map((o) => (
              <tr key={o.id} className="border-t border-border">
                ...
                <td className="p-3 text-muted-foreground">{o.customer?.user.email ?? o.guestEmail ?? 'guest'}</td>
                ...
                <td className="p-3 text-end"><Link href={`/admin/orders/${o.id}`} className="text-primary hover:underline">Open</Link></td>
              </tr>
            ))}
            {orders.length === 0 && <tr><td colSpan={9} className="p-6 text-center text-muted-foreground">No orders.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
