  ...
  return (
    <div className="p-6">
      <header className="mb-6">
        <h1 className="font-heading text-xl font-semibold">Return · order {ret.order.number}</h1>
        <p className="text-sm text-muted-foreground">Reason: {ret.reasonCode} · <StatusBadge status={ret.status} /></p>
      </header>
      ...
      <form action={processReturnAction} className="max-w-2xl space-y-5">
        ...
        <div className="space-y-2">
          <p className="text-sm font-medium">Returned items — set disposition (quarantine for pharmacist review, never auto-resell)</p>
          {ret.items.map((ri) => (
            <div key={ri.id} className="flex items-center justify-between rounded-md border border-border p-3 text-sm">
              ...
              <span className="flex items-center gap-2">
                ...
                <select name="disposition" defaultValue={ri.disposition} className={`${inputCls} w-40`}>
                  ...
                  <option value="RESTOCK">Restock → quarantine</option>
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <label className="block text-sm font-medium">Refund method
            <input name="refundMethod" placeholder="bank transfer / store credit" className={inputCls} />
          </label>
          <label className="block text-sm font-medium">Refund (EGP)
            <input name="refundEgp" type="number" step="0.01" min="0" className={inputCls} />
          </label>
        </div>

        <button className="rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground">Process return</button>
      </form>
    </div>
  );
}
