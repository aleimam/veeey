...
export default async function ComparePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  if (products.length === 0) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-20 text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">{t('emptyNote')}</p>
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      <h1 className="mb-6 font-heading text-2xl font-semibold text-foreground">{t('title', { count: products.length })}</h1>
      <div className="overflow-x-auto">
        <table className="w-full border-collapse text-sm">
          <tbody>
            ...
            <tr>
              <td className="border border-border bg-surface p-3 font-medium">{t('brand')}</td>
              {products.map((p) => <td key={p.id} className="border border-border p-3">{p.brand?.nameEn ?? '—'}</td>)}
            </tr>
            <tr>
              <td className="border border-border bg-surface p-3 font-medium">{t('price')}</td>
              {products.map((p) => <td key={p.id} className="border border-border p-3">{formatEGP(Number(p.basePricePiastres))}</td>)}
            </tr>
            <tr>
              <td className="border border-border bg-surface p-3 font-medium">{t('rating')}</td>
              {products.map((p) => <td key={p.id} className="border border-border p-3">{(p.ratingAvg ?? 0).toFixed(1)} ({p.ratingCount})</td>)}
            </tr>
            <tr>
              <td className="border border-border bg-surface p-3 font-medium">{t('inStock')}</td>
              {products.map((p) => <td key={p.id} className="border border-border p-3">{p.lots.some((l) => l.qtyOnHand > 0) ? t('yes') : t('preorder')}</td>)}
            </tr>
            {attrNames.map((name) => (
              <tr key={name}>
