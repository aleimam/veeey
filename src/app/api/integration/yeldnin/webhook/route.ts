...
import { dispatchInboundEvent } from '@/lib/integration/integration-handlers';

/**
 * YeldnIN → Veeey webhook (INTEGRATION_CONTRACT §5). 404 while the flag is off.
 * HMAC-verified (§1), nonce-replay-protected, idempotent (§2).
 */
const ENDPOINT = 'yeldnin.webhook';
...
export async function POST(req: Request) {
  ...
  const v = await verifyInbound({
    ...
    path: url.pathname,
    rawBody: raw,
    headers: { clientId: req.headers.get('x-client-id'), timestamp: req.headers.get('x-timestamp'), nonce: req.headers.get('x-nonce'), signature: req.headers.get('x-signature') },
    nowMs: Date.now(),
  });
  ...
  let body: { id?: number; type?: string; occurredAt?: string; payload?: unknown };
  try {
    body = JSON.parse(raw || '{}');
  } catch {
    return err('validation_failed', 422);
  }
  if (!body.type) return err('validation_failed', 422);

  const result = await dispatchInboundEvent(String(body.type), body.payload);
  const response = { ok: true, type: body.type, ...result };
