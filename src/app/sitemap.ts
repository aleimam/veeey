...
const STATIC = ['', '/products', '/brands', '/collections', '/play', '/blog', '/search'];
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    const [products, posts, cmsPages, brands, collections] = await Promise.all([
      ...
      // Brands: public = not archived + indexable. No updatedAt column → createdAt.
      prisma.brand.findMany({ where: { archivedAt: null, robotsIndex: true }, select: { slug: true, createdAt: true }, take: 2000 }),
      prisma.collection.findMany({ where: { status: 'PUBLISHED' }, select: { slug: true, updatedAt: true }, take: 500 }),
    ]);
    for (const l of LOCALES) {
      ...
      for (const b of brands) entries.push({ url: `${SITE_URL}/${l}/brands/${b.slug}`, lastModified: b.createdAt, changeFrequency: 'weekly', priority: 0.6 });
      for (const c of collections) entries.push({ url: `${SITE_URL}/${l}/collection/${c.slug}`, lastModified: c.updatedAt, changeFrequency: 'weekly', priority: 0.6 });
