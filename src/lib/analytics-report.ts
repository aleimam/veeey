...
import { resolveAnalyticsRange, type ResolvedRange } from '@/lib/analytics-range';
...
export type ReportConfig = { dimension: DimKey; metric: MetricKey; days: number; endAt?: Date; range: ResolvedRange; filterDim: DimKey | null; filterVal: string };
...
export function resolveReportConfig(raw: { dimension?: string; metric?: string; days?: string; preset?: string; from?: string; to?: string; fdim?: string; fval?: string }): ReportConfig {
  ...
  // Shared range contract with the analytics dashboard + Sales (V5 audit
  // F10/F11): presets + custom from/to, legacy ?days= still honored.
  const range = resolveAnalyticsRange(raw, { defaultPreset: '30d' });
  ...
  return { dimension, metric, days: range.days, endAt: range.endAt, range, filterDim, filterVal };
}
...
export async function runReport(cfg: ReportConfig): Promise<ReportRow[]> {
  ...
  const sql = `
    ...
    WHERE e."createdAt" >= $1 AND e."createdAt" <= $2 AND s."isBot" = false${filterSql}
